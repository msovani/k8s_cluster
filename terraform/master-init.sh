...
echo "$(date): kubeadm installed" >> /tmp/custom-user-data.log
sudo kubeadm init --pod-network-cidr 192.168.0.0/16 --kubernetes-version ${kubernetes_short_version}
#sudo kubeadm init --apiserver-advertise-address=192.168.56.2 --pod-network-cidr 192.168.0.0/16 --kubernetes-version 1.25.10

echo "$(date): kubeadm init complete" >> /tmp/custom-user-data.log
mkdir -p $HOME/.kube
sudo cp -i /etc/kubernetes/admin.conf $HOME/.kube/config
sudo chown $(id -u):$(id -g) $HOME/.kube/config

echo "$(date): kubeconfig configured" >> /tmp/custom-user-data.log
kubectl create -f https://raw.githubusercontent.com/projectcalico/calico/v3.25.1/manifests/tigera-operator.yaml

kubectl create -f https://raw.githubusercontent.com/projectcalico/calico/v3.25.1/manifests/custom-resources.yaml

echo "$(date): calico plugin installed" >> /tmp/custom-user-data.log
kubeadm token create --print-join-command >> /tmp/custom-user-data.log


sudo apt-get install -y etcd-client
echo "$(date): etcd-client installed" >> /tmp/custom-user-data.log
...
echo "$(date): kube-bench installed" >> /tmp/custom-user-data.log
